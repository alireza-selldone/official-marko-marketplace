...
Promise.all([loadCatalog(), loadVendors().catch(() => [])]).then(([catalog, liveVendors]) => {
  ...
  set("[data-vendor-product-count]", products.length);
  set("[data-vendor-category-count]", categories.length);
  ...
  if (art) art.innerHTML = products.slice(0, 6).map((product, index) => `<a class="vendor-hero__product vendor-hero__product--${index + 1}" href="product.html?id=${product.id}"><img src="${esc(product.image)}" alt="${esc(product.name)}" width="420" height="420"><span>${esc(product.name)}</span></a>`).join("");

  const categoryGrid = document.querySelector("[data-vendor-categories]");
  if (categoryGrid) categoryGrid.innerHTML = categories.map((category) => `<a href="shop.html?cat=${encodeURIComponent(category.slug)}"><span><img src="${esc(category.image)}" alt="" loading="lazy" width="300" height="300"></span><b>${esc(category.name)}</b><small>${category.count} products</small></a>`).join("");
